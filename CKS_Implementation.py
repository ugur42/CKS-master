...
#This is a modified version of Qiskit HHL algorithm, attempting to implement
#the Childs Kothari Somma algorithm (CKS)
...
#Hsim used to implement matrix A as a circuit and not to invert x
...
# Following code is used to get the condition number of the matrix with NumPyMatrix.condition_bounds()
...
#note that int(np.ceil(np.log2(kappa + 1))) corresponds to m in CKS
        nl = max(nb + 1, int(np.ceil(np.log2(kappa + 1)))) + neg_vals
        
        
#generate circuit setting j = m
# 
#determine the number of qubits nj used for GPE and W for cycle-varible j, with nj <= nl
# 
#cycle the following code, such that phi, l and nj are calculated and applied to the circuit to construct A_j
        # delta = self._epsilon/kappa #matching delta in CKS page 25 bottom half
# """
# # set delta and ancilla register following it
# """
        qc = 0
        j = 0
        nj = 1
        while nj < nl:
            phi = 2**-j
            l = int(np.ceil(int(np.log(1/phi)))) 
            nj = 1+l
            j = j+1
#instead of having maximal precision of eigenvalues with nl, we use smaller values than
#nl to produce GPE (low precision voersion of QPE) and apply 1/x in those ranges
    ...
    # cant use the next line for CKS as it implements 1/x directly and not depending on interval
    # what is the cost in that case?
            if self._exact_reciprocal:
                reciprocal_circuit = ExactReciprocal(nj, delta, neg_vals=neg_vals)
                # Update number of ancilla qubits
                na = matrix_circuit.num_ancillas
    #following lines could be used in CKS, to implement 1/x with qiskits class PiecewiseChebyshev
    # also suggested in the CKS paper       
            else:
    ...
    #1/x instead os arcsin(constant/x) as was in OG HHL in the following line
                if j==1:
                    reciprocal_circuit = PiecewiseChebyshev(
                        lambda x: 1 / x, degree, breakpoints, nl
                    )
                    na = max(matrix_circuit.num_ancillas, reciprocal_circuit.num_ancillas)
                else:
                    reciprocal_circuit = PiecewiseChebyshev(
                        lambda x: 1 / x, degree, breakpoints, nj
                    )
                    na = max(matrix_circuit.num_ancillas, reciprocal_circuit.num_ancillas)
    

#generate circuit with maxcimal nr of qubits
            if j == 1:
                # Initialise the quantum registers
                qb = QuantumRegister(nb)  # right hand side and solution
                ql = QuantumRegister(nl)  # eigenvalue evaluation qubits
 #add clock register? or used nl qubits for this? CKS uses c and p register, in HHL this is both summed into n_l?!
                if na > 0:
            ...
            if j == 1:
                ...
                pass
            # GPE
    #perform GPE for increasing size of qubits nj, with nj <= nl
            #number of qubits for GPE depending on cycle-value 2**-j
            #see Lemma 22 CKS 
            if j==1:
                continue
            else:
                phase_estimation = PhaseEstimation(nj, matrix_circuit,name="GPE")
                if na > 0:
                    qc.append(phase_estimation, ql[:nj] + qb[:] + qa[: matrix_circuit.num_ancillas])
                else:
                    qc.append(phase_estimation, ql[:nj] + qb[:])
                ...
                if self._exact_reciprocal:
                    ...
                    qc.append(
                        ...
                        ql[:nj] + [qf[0]] + qa[: reciprocal_circuit.num_ancillas],
                    )
                # QPE inverse
                if na > 0:
                    qc.append(phase_estimation.inverse(), ql[:nj] + qb[:] + qa[: matrix_circuit.num_ancillas])
                else:
                    qc.append(phase_estimation.inverse(), ql[:nj] + qb[:])
        return qc
